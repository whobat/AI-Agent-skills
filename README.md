...
| **ax2012-aos-crash-triage** | PowerShell 7+ (auto-installed) | — | Admin credential on the targets, **prompted each run**. WinRM on the AOS + client/RDS hosts. Read-only. |
...
| **ax2012-aos-crash-triage** | **Dynamics AX 2012 (R3) AOS crash & session-cascade triage** over WinRM: collects AOS process crashes (Ax32Serv 1000), unexpected terminations (7031), and the kernel session-allocation precursors (Dynamics Server 110/180), then correlates AOS terminations with Ax32.exe rich-client crashes on the RDS farm to expose the "AOS dies → clients all drop → reconnect storm" cascade. Read-only; window computed server-side (TZ-safe). | PowerShell 7+ (auto-installed), WinRM on the AOS + client/RDS hosts, admin credential (prompted). |
