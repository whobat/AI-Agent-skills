...
- **A skill is the folder `skills/<category>/<name>/`** with a `SKILL.md` whose frontmatter follows the [Agent Skills specification](https://agentskills.io/specification) **strictly**: only spec-defined fields (`name`, `description`, `license`, `compatibility`, `metadata`, `allowed-tools`); the version lives under `metadata.version`. Skills are grouped into category folders for repo overview, but `name` must equal the skill's **own** folder (not the category), and **names are globally unique** (they install flat into one namespace). Every category folder must be listed in `.claude-plugin/plugin.json`'s `skills` array (else the marketplace misses it). The installers discover skills by their `SKILL.md` (any depth ≤ 2) and install them **flat** into the agent dir. Every skill must also carry a **`## Gotchas` section** (domain pitfalls so agents don't repeat known misdiagnoses) and a **`## Verification` section** (how to verify correctness *before* changing anything, and how to verify the *output* afterward) — both in `SKILL.md` or `REFERENCE.md`, both validator-enforced. Committed gotchas stay **generic**; environment-specific ones go in a gitignored, update-preserved per-skill `gotchas.local.md` the agent reads and appends to (so a skill improves after first use). `node tests/validate-skills.mjs` (also `npm run validate` / CI) enforces all of this and must pass before every commit that touches a skill. Owning rules live in [skills/AGENTS.md](skills/AGENTS.md).
